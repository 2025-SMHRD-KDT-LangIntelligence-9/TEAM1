...
class ScannerApp(ctk.CTk):
    def __init__(self):
        ...
        # --- 1. 초기 창 설정 ---
        self.scanner_w, self.scanner_h = 280, 170  # 스캐너 창의 가로, 세로 크기 지정
    ...
    def setup_ui(self):
        """스캐너 내부의 버튼, 스위치 등 디자인 요소를 배치하는 함수"""
        
        # 전체를 감싸는 둥근 모서리의 짙은 회색 배경 카드 프레임
        main_card = ctk.CTkFrame(self, corner_radius=20, fg_color="#1e1e1e", border_width=1, border_color="#3d3d3d")
        main_card.pack(pady=10, padx=10, fill="both", expand=True) # 화면에 꽉 차게 배치

        # --- 상단 컨트롤 라인 (스위치, 메인버튼, 투명도 조절) ---
        top = ctk.CTkFrame(main_card, fg_color="transparent") # 배경이 투명한 프레임
        top.pack(fill="x", padx=15, pady=15) # 가로로 꽉 차게 배치

        # 1. 자동 감지 필터 켜기/끄기 스위치
        self.switch_filter = ctk.CTkSwitch(top, text="ON", font=("맑은 고딕", 12, "bold"), 
                                           command=self.toggle_filter, progress_color="#10b981", width=50)
        self.switch_filter.pack(side="left") # 왼쪽에 배치

        # 2. 메인 화면으로 돌아가는 버튼 (close_and_restore_main 함수 연결)
        ctk.CTkButton(top, text="메인", font=("맑은 고딕", 11), fg_color="#3d3d3d", 
                      width=50, height=24, command=self.close_and_restore_main).pack(side="left", padx=10)

        # 3. 창의 투명도를 조절하는 슬라이더 (0.3 ~ 1.0 사이 값 조절)
        self.slider_alpha = ctk.CTkSlider(top, from_=0.3, to=1.0, width=90, 
                                          command=lambda v: self.attributes('-alpha', v))
        self.slider_alpha.set(1.0)           # 기본값은 투명도 100% (불투명)
        self.slider_alpha.pack(side="right") # 오른쪽에 배치
        ...
        self.btn_scan.pack(expand=True) # 남은 공간 중앙에 배치
        
        # --- 하단 현재 상태 알림 텍스트 ---
        self.lbl_status = ctk.CTkLabel(main_card, text="OFF: Manual Mode", font=("맑은 고딕", 12), text_color="#a1a1aa")
        self.lbl_status.pack(side="bottom", pady=10) # 맨 아래에 배치
