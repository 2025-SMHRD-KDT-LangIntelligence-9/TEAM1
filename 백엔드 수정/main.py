...
async def read_root(request: Request):
    # 1. 브라우저 쿠키에서 'user_session'이라는 로그인 토큰을 꺼냅니다.
    token = request.cookies.get("user_session")
    is_logged_in = token is not None
    
    # 2. 🌟 [핵심] index.html 화면을 띄울 때, 'access_token'이라는 이름으로 실제 토큰 값을 꽉꽉 채워서 넘겨줍니다!
    return templates.TemplateResponse(
        request=request, 
        name="index.html", 
        context={
            "is_logged_in": is_logged_in,
            "access_token": token if token else ""  # 👈 바로 이 한 줄이 추가되었습니다!
        }
    )
...
# [추가] 스캐너로부터 분석 요청을 받아 진짜 백엔드(0.77)로 전달해주는 중계 로직
@app.post("/analyze")
async def proxy_analyze(request: Request):
    try:
        ...
        # 2. 헤더 설정 (토큰이 없으면 마스터 키 사용)
        # 만약 스캐너에서 토큰이 안 왔다면, 중계 서버가 백엔드에 미리 약속된 마스터 키를 보냅니다.
        if not token or token == "None":
            # [방법 A] 백엔드에서 'Scanner-Auth'라는 특수 헤더를 받도록 설정되어 있다면 사용
            # [방법 B] 가장 편한 방법: 토큰이 없으면 그냥 빈 헤더를 보내고, 
            # 대신 백엔드(.77번 PC)의 /analyze 주소에서 인증(Depends)을 제거합니다.
            headers = {} 
        else:
            headers = {"Authorization": token}

        # 3. 진짜 백엔드로 전달
        # 주의: 백엔드(.77)에서도 이 요청을 받을 때 에러가 안 나려면 인증을 풀어둬야 합니다.
        res = requests.post(
            f"{BACKEND_URL}/analyze", 
            json=data, 
            headers=headers, # 정리된 헤더 사용
            timeout=15
        )
        ...
        print(f"[중계 에러] 백엔드 연결 실패: {e}")
        return JSONResponse(status_code=500, content={"message": "중계 서버 오류"})

# [추가] 스캐너로부터 저장 요청을 받아 진짜 백엔드로 전달해주는 중계 로직
@app.post("/save")
