...
class FinalApp(ctk.CTk):
    ...
    def show_popup(self, original_text, corrections, context_type):
        ...
        popup.attributes('-topmost', True) # 팝업을 항상 최상단에 둠
        
        popup_width, popup_height = 400, 280
        # 마우스 커서의 현재 위치를 기준으로 4.5cm(약 170px) 위에 배치
        mx, my = pyautogui.position()
        popup.geometry(f"{popup_width}x{popup_height}+{mx - 200}+{my - 170 - popup_height}")
